  ...
  return (
    <div>
      <h2 id="cotisation-heading" data-cy="CotisationHeading">
        <Translate contentKey="adpaievfbApp.cotisation.home.title">Cotisations</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adpaievfbApp.cotisation.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/cotisation/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="adpaievfbApp.cotisation.home.createLabel">Create new Cotisation</Translate>
          </Link>
        </div>
      </h2>
        ...
        {cotisationList && cotisationList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="adpaievfbApp.cotisation.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {cotisationList.map((cotisation, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`adpaievfbApp.Categorie.${cotisation.famille}`} />
                  </td>
                  ...
                  <td>{cotisation.multiplicateurMin}</td>
                  <td>{cotisation.multiplicateurMax}</td>
                  <td>
                    {cotisation.baseTauxCotisation ? (
                      <Link to={`/base-taux-cotisation/${cotisation.baseTauxCotisation.id}`}>{cotisation.baseTauxCotisation.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adpaievfbApp.cotisation.home.notFound">No Cotisations found</Translate>
            </div>
          )
        )}
      </div>
