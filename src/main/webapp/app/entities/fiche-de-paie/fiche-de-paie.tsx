  ...
  return (
    <div>
      <h2 id="fiche-de-paie-heading" data-cy="FicheDePaieHeading">
        <Translate contentKey="adpaievfbApp.ficheDePaie.home.title">Fiche De Paies</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adpaievfbApp.ficheDePaie.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/fiche-de-paie/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="adpaievfbApp.ficheDePaie.home.createLabel">Create new Fiche De Paie</Translate>
          </Link>
        </div>
      </h2>
        ...
        {ficheDePaieList && ficheDePaieList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="adpaievfbApp.ficheDePaie.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {ficheDePaieList.map((ficheDePaie, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {ficheDePaie.mentionsObligatoires
                      ? ficheDePaie.mentionsObligatoires.map((val, j) => (
                          <span key={j}>
                            <Link to={`/mentions-obligatoires/${val.id}`}>{val.id}</Link>
                            {j === ficheDePaie.mentionsObligatoires.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adpaievfbApp.ficheDePaie.home.notFound">No Fiche De Paies found</Translate>
            </div>
          )
        )}
      </div>
