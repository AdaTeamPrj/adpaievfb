  ...
  return (
    <div>
      <h2 id="conge-heading" data-cy="CongeHeading">
        <Translate contentKey="adpaievfbApp.conge.home.title">Conges</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adpaievfbApp.conge.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/conge/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="adpaievfbApp.conge.home.createLabel">Create new Conge</Translate>
          </Link>
        </div>
      </h2>
        ...
        {congeList && congeList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="adpaievfbApp.conge.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {congeList.map((conge, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`adpaievfbApp.Decision.${conge.decision}`} />
                  </td>
                  <td>{conge.dateReponse ? <TextFormat type="date" value={conge.dateReponse} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>
                    <Translate contentKey={`adpaievfbApp.TypeConge.${conge.typeConge}`} />
                  </td>
                  <td>{conge.congePay}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adpaievfbApp.conge.home.notFound">No Conges found</Translate>
            </div>
          )
        )}
      </div>
