...
public class FicheDePaieRepositoryWithBagRelationshipsImpl implements FicheDePaieRepositoryWithBagRelationships {
    ...
    public Optional<FicheDePaie> fetchBagRelationships(Optional<FicheDePaie> ficheDePaie) {
        return ficheDePaie.map(this::fetchCotisations).map(this::fetchMentionsObligatoires);
    }
    ...
    public List<FicheDePaie> fetchBagRelationships(List<FicheDePaie> ficheDePaies) {
        return Optional.of(ficheDePaies).map(this::fetchCotisations).map(this::fetchMentionsObligatoires).orElse(Collections.emptyList());
    }
    ...
    FicheDePaie fetchMentionsObligatoires(FicheDePaie result) {
        return entityManager
            .createQuery(
                "select ficheDePaie from FicheDePaie ficheDePaie left join fetch ficheDePaie.mentionsObligatoires where ficheDePaie is :ficheDePaie",
                FicheDePaie.class
            )
            .setParameter("ficheDePaie", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
    ...
    List<FicheDePaie> fetchMentionsObligatoires(List<FicheDePaie> ficheDePaies) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, ficheDePaies.size()).forEach(index -> order.put(ficheDePaies.get(index).getId(), index));
        List<FicheDePaie> result = entityManager
            .createQuery(
                "select distinct ficheDePaie from FicheDePaie ficheDePaie left join fetch ficheDePaie.mentionsObligatoires where ficheDePaie in :ficheDePaies",
                FicheDePaie.class
            )
            .setParameter("ficheDePaies", ficheDePaies)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
